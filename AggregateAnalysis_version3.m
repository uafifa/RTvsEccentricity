...
[protocolNames, colors, linestyle, axes, refDist] = experimentInformation();
S = {'o', 'd', 's', 'h', '*', '^'};
refDist = str2num(refDist{1});
...
for ii = 1:length(outputParamStats)
    ...
    fileName = strcat('Protocol ', string(ii), 'output-Parameters for' , {' '}, ...
...
for ii = 1:length(protocolNames)
    protocolChiFitPos(ii) = chiSquareFunction(masterAngles(masterAngles >= refDist)...
        ,protocolWeightedMean(ii,find(masterAngles >= refDist))...
        ,protocolStdErr(ii,find(masterAngles >= refDist)));
    protocolChiFitNeg(ii) = chiSquareFunction(masterAngles(masterAngles <= refDist)...
        ,protocolWeightedMean(ii,find(masterAngles <= refDist))...
        ,protocolStdErr(ii,find(masterAngles <= refDist)));
end
...
for ii = 1:length(protocolNames)
    ...
    protocolChiFit(ii).interceptPos = protocolChiFitPos(ii).slope;
...
plotProtocolData_v3(fileList,listing,...
    ...
    protocolStdErr,protocolChiFitPos, protocolChiFitNeg, direc)
%% ANOVA Analysis
 %Change based on what experiment you are analyzing
task = ["SRT", "SRT", "CRT", "CRT"];
subject = string(1:length(angleRT_Mean(1).protocol));
angles = ["-60" ,  "-45" , "-30" ,  "-15"   ,  "0" ,   "15" ,    "30",    "45"  ,  "60"];
attn = ["Covert", "Overt", "Covert", "Overt"];
...
for ii = 1:length(angleRT_Mean)
    for jj = 1:length(angleRT_Mean(1).protocol)
        for kk = 1:length(angleRT_Mean(1).protocol(1).subject)
            RT_mat = [RT_mat; angleRT_Mean(ii).protocol(jj).subject(kk).data];
            angles_mat = [angles_mat; angles(kk) ];
            subject_mat = [subject_mat; subject(jj)];
            task_mat = [task_mat; task(ii) ];
            attn_mat = [attn_mat; attn(ii)];
        end 
    end 
end

data_tbl = table(subject_mat, task_mat, attn_mat, angles_mat, RT_mat);
data_tbl.Properties.VariableNames = {'Subject', 'Task', 'Attn', 'Angles', 'RT'};

%Linear Mixed Effects Model

    lme = fitlme(data_tbl, 'RT~ Angles*Task*Attn + (1|Subject)');
    stats = anova(lme);
    %altlme = fitlme(data_tbl, 'RT~ Angles*Task*Attn + (1+Angles*Task*Attn|Subject)');
    cd(dirName);
writetable(table(dataset2cell(stats)), 'LME_Attn_Task_Angles_all.xlsx');
